...
def main() -> None:
    if len(sys.argv) == 1:
        ...
        return
    parser = OptionParser(
        "sxtool.py [options] [filename1, ..., filenameN]"
    )  # type: 'OptionParser'
    parser.add_option(
        "-w",
        "--window",
    ...
    parser.add_option(
        "--version", action="store_true", dest="version", help="Show tool version"
    )
    parser.add_option(
        "-m",
        "--mask",
    ...
    if options.window or (
        not options.flipBits and not options.offset and not options.xorData
    ):
        startWinMode(args)
        return
