...
def main() -> None:
    ...
    parser.add_option(
        '-w', '--window', action='store_true', dest='window', default=False, help='Start the application in window mode'
    )
    ...
    parser.add_option('-o', '--offset', dest='offset', help='Apply an integer offset on all adresses of the records')
    parser.add_option('--version', action='store_true', dest='version', help='Show tool version')
    parser.add_option(
        '-m', '--mask', dest='xorData', help='Apply a XOR hexadecimal mask on all data fields of the records'
    )
...
def apply(files: List[str], operation: str, args: Optional[List[str]] = None, verbose: bool = False) -> None:
    ...
    for file in files:   # type: str
        try:
            ...
            continue
        if verbose:
            print('Applying operation "' + operation + '" on file:')
