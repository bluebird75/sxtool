...
class TestWithGui(unittest.TestCase):
    ...
    def testInsertFile(self):
        ...
        # Refuse insertion
        mockDialog = Mock(
            exec_=lambda: QDialog.Rejected, windowFlags=lambda: 0, radioStart=Mock(isChecked=lambda: True)
        )
        ...
        mockDialog = Mock(
            exec_=lambda: QDialog.Accepted, windowFlags=lambda: 0, radioStart=Mock(isChecked=lambda: True)
        )
        w.insertFname(str(SX_EXAMPLE1), mockDialog)
