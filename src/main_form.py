...
class MainForm(Ui_MainForm, QMainWindow):  # type: ignore # PyQt and Mypy don't mix very well
    ...
    def slotOpen(self) -> None:
        ...
        fname, selectedFilter = QFileDialog.getOpenFileName(
            self, "Choose file", '', SX_FILES_PATTERNS)  # type: str, str
        if not fname:
    ...
    def slotInsert(self) -> None:
        ...
        fname, filterName = QFileDialog.getOpenFileName(
            self, "Insert file", self.lastDir[0], SX_FILES_PATTERNS)  # type: str, str
    ...
    def slotSaveAs(self) -> None:
        ...
        fname, filterName = QFileDialog.getSaveFileName(
            self, "Save file as", self.lastDir[0], SX_FILES_PATTERNS)  # type: str, str
        if not fname:
            return
    ...
    def slotDelete(self) -> None:
        ...
        answer = QMessageBox.question(
            self, "Are you sure ?", "Delete these %d lines ?" %
            i, QMessageBox.Yes, QMessageBox.No)
