...
def ensureAnyLinesAreSelected(
    f: Callable[["MainForm"], None]
) -> Callable[["MainForm"], None]:
    @wraps(f)
...
def ensureDataLinesAreSelected(
    f: Callable[["MainForm"], None]
) -> Callable[["MainForm"], None]:
    @wraps(f)
    def wrapper(self: "MainForm") -> None:
...
class MainForm(Ui_MainForm, QMainWindow):  # type: ignore # PyQt and Mypy don't mix very well
    def __init__(
        ...
        self.menuHistory = ItemHistoryMenu(
            fileName="open_recent_history.xml",
            maxSize=10,
            menu=self.menuOpenRecent,
            callbackMethod=self.loadFile,
        )
        self.dirHistory = ItemHistoryStringList(
            fileName="last_dir.xml", maxSize=1, stringList=self.lastDir
        )
        if not self.lastDir:
    ...
    def slotOpen(self) -> None:
        ...
        fname, selectedFilter = QFileDialog.getOpenFileName(
            self, "Choose file", "", SX_FILES_PATTERNS
        )  # type: str, str
        if not fname:
    ...
    def loadFile(self, fname: str) -> bool:
        ...
        self.resize(
            min(goodSize[0], reasonableSize[0]), min(goodSize[1], reasonableSize[1])
        )
        return True

    def slotInsert(self) -> None:
        self.statusBar().showMessage("Inserting file ...")
        fname, filterName = QFileDialog.getOpenFileName(
            self, "Insert file", self.lastDir[0], SX_FILES_PATTERNS
        )  # type: str, str
    ...
    def slotSave(self) -> bool:
        ...
        if not self.dataTable:
            QMessageBox.critical(
                None, "Internal Error o_O", "An internal error occured."
            )
            return False
        try:
        ...
        self.statusBar().showMessage(
            "File %s successfully saved !" % self.dataTable.file
        )
    ...
    def slotSaveAs(self) -> None:
        ...
        if not self.dataTable:
            ...
            return
        fname, filterName = QFileDialog.getSaveFileName(
            self, "Save file as", self.lastDir[0], SX_FILES_PATTERNS
        )  # type: str, str
        if not fname:
            return
    ...
    def slotInsertRow(self) -> None:
        ...
        default_format = (
            default_format[0] + str(10 - int(default_format[1])) + default_format[1]
        )
    ...
    def slotEditData(self) -> None:
        ...
        val = QInputDialog.getText(
            self, "Enter new data for row(s)", "Enter new data:", QLineEdit.Normal, text
        )  # type: Tuple[str,bool]
        text = val[0]
        ok = val[1]  # type: bool
        if len(text) % 2 != 0:
    ...
    def slotSplitRow(self) -> None:
        ...
        rowList = [
            r for r in allRowList if 0 < r < 1 + len(self.dataTable.sxfile.sxItems)
        ]
    ...
    def slotMergeRows(self) -> None:
        ...
        if self.dataTable.numRowsSelected() < 2:
            QMessageBox.warning(
                self,
                "Not enough lines",
                "You must select at least two line to merge them!",
            )
            return
    ...
    def slotVerifyChecksum(self) -> None:
        ...
        if len(invalidChecksumRows):
            ...
            for rowNb, address, validChecksum, wrongChecksum in invalidChecksumRows:
                msgList.append(
                    "Invalid checksum for address %s (line %d): got %s instead of %s"
    ...
    def slotRecalculateChecksum(self) -> None:
        ...
        if nbInvalidChecksums > 0:
            ...
            QMessageBox.information(
                self, "Checksum recalculation", "All checksums were already valid"
            )
