    ...
    def wrapper(self: 'MainForm') -> None:
        if self.dataTable.numRowsSelected() == 0:
            QMessageBox.warning(self, "No selection",
                                "You must select lines before applying this operation, aborting.")
            return
        return f(self)
...
def ensureDataLinesAreSelected(f: Callable[['MainForm'], None]) -> Callable[['MainForm'], None]:
    ...
    def wrapper(self: 'MainForm') -> None:
        ...
        if 0 in selRows or self.dataTable.rowCount() - 1 in selRows:
            QMessageBox.warning(self, "First or last line selected",
                                "This operation does not work on first or last lines, aborting.")
            return
        return f(self)
...
class MainForm(Ui_MainForm, QMainWindow):  # type: ignore # PyQt and Mypy don't mix very well
    def __init__(self,
        ...
        self.menuHistory = ItemHistoryMenu(fileName="open_recent_history.xml",
                                           maxSize=10,
                                           menu=self.menuOpenRecent,
                                           callbackMethod=self.loadFile)
        ...
        self.actionList = [
            self.insertAction, self.saveAction, self.saveasAction, self.copyAction, self.cutAction,
            self.deleteAction, self.convertToAction, self.convertToS19Action, self.convertToS28Action,
            self.convertToS37Action, self.selectallAction, self.insertRowAction, self.deleteRowAction,
            self.editDataAction, self.convertToS19Action, self.convertToS28Action, self.convertToS37Action,
            self.applyOffsetAction, self.mergeRowsAction, self.splitRowAction, self.setRowSizeAction,
            self.xorRowAction, self.flipBitsAction
        ]  # type: List[Any]
    ...
    def slotOpen(self) -> None:
        ...
        fname, selectedFilter = QFileDialog.getOpenFileName(self, "Choose file", '',
                                                            SX_FILES_PATTERNS)  # type: str, str
        if not fname:
    ...
    def slotSaveAs(self) -> None:
        ...
        if not self.dataTable:
            QMessageBox.critical(None, "Internal Error o_O", "An internal error occured.")
            return
        fname, filterName = QFileDialog.getSaveFileName(self, "Save file as", self.lastDir[0],
                                                        SX_FILES_PATTERNS)  # type: str, str
        if not fname: return
    ...
    def slotDelete(self) -> None:
        ...
        answer = QMessageBox.question(self, "Are you sure ?", "Delete these %d lines ?" % i, QMessageBox.Yes,
                                      QMessageBox.No)
    ...
    def slotEditData(self) -> None:
        ...
        val = QInputDialog.getText(self, "Enter new data for row(s)", "Enter new data:", QLineEdit.Normal,
    ...
    def slotEditRowData(self, item: Any) -> None:
        ...
        val = QInputDialog.getText(self,
                                   "Enter new data for row(s)",
                                   "Enter new data:",
                                   QLineEdit.Normal,
                                   item.text(),
                                   flags=Qt.WindowTitleHint
                                   | Qt.WindowCloseButtonHint)  # type: Tuple[str,bool]
    ...
    def slotApplyOffset(self) -> None:
        ...
        val = QInputDialog.getInt(self,
                                  "Which Offset ?",
                                  "Which offset do you want to apply to addresses ?",
                                  1,
                                  flags=Qt.WindowTitleHint
                                  | Qt.WindowCloseButtonHint)  # type: Tuple[int,bool]
    ...
    def slotAbout(self) -> None:
        QMessageBox.about(self, "SXFiles Manipulator", ABOUT_INFO)
    ...
    def slotVerifyChecksum(self) -> None:
        ...
        if len(invalidChecksumRows):
            ...
            for rowNb, address, validChecksum, wrongChecksum in invalidChecksumRows:
                msgList.append('Invalid checksum for address %s (line %d): got %s instead of %s' %
                               (address, rowNb + 1, wrongChecksum, validChecksum))
            if len(invalidChecksumRows) > 10:
                msgList.append('...')
            QMessageBox.warning(self, "Checksum verification failed",
    ...
    def slotRecalculateChecksum(self) -> None:
        ...
        if nbInvalidChecksums > 0:
            QMessageBox.information(self, "Checksum recalculation",
                                    "%d checksums adjusted." % nbInvalidChecksums)
        else:
