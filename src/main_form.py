...
def ensureDataLinesAreSelected(f: Callable[['MainForm'], None]) -> Callable[['MainForm'], None]:
    ...
    def wrapper(self: 'MainForm') -> None:
        ...
        if 0 in selRows or self.dataTable.rowCount() - 1 in selRows:
            QMessageBox.warning(
                self, "First or last line selected", "This operation does not work on first or last lines, aborting."
            )
            return
        return f(self)
...
class MainForm(Ui_MainForm, QMainWindow):  # type: ignore # PyQt and Mypy don't mix very well
    def __init__(
        self, parent: Optional[QWidget] = None, name: str = "SX Tool", fl: int = 0, file: Optional[str] = None
    ) -> None:
        ...
        self.menuHistory = ItemHistoryMenu(
            fileName="open_recent_history.xml", maxSize=10, menu=self.menuOpenRecent, callbackMethod=self.loadFile
        )
    ...
    def slotDelete(self) -> None:
        ...
        answer = QMessageBox.question(
            self, "Are you sure ?", "Delete these %d lines ?" % i, QMessageBox.Yes, QMessageBox.No
        )
    ...
    def slotSetRowSize(self) -> None:
        if not self.dataTable.isSelectionContinuus():
            QMessageBox.critical(
                self, "Non continuus selection", "You must select one range of row for this action to work"
            )
            return
    ...
    def slotVerifyChecksum(self) -> None:
        ...
        if len(invalidChecksumRows):
            ...
            if len(invalidChecksumRows) > 10:
                msgList.append('...')
            QMessageBox.warning(
                self, "Checksum verification failed", "Checksums verification failed!\n\n" + "\n".join(msgList)
            )
        else:
            if len(rows) > 1:
