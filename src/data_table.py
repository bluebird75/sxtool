...
class DataTable(QTableWidget):  # type: ignore # PyQt and Mypy don't mix very well
    ...
    def __init__(
        self, parent: Optional[QWidget] = None, name: Optional[str] = None
    ) -> None:
    ...
    def initTable(self) -> None:
        ...
        for i in range(5):
            self.horizontalHeader().resizeSection(
                ...
                int(
                    max(
                        self.sizeHintForColumn(i),
                        self.horizontalHeader().sectionSizeHint(i),
                    )
                    * 1.1
                ),
            )
    ...
    def loadFile(self, fname: str) -> bool:
        ...
        try:
            ...
            QMessageBox.critical(
                None, "Error", "Could not open file %s\n%s" % (self.file, repr(e))
            )
            return False
        self.redisplayTable()
    ...
    def insertRowsWithData(self, dialog: FormInsertRowValue) -> None:
        ...
        data = ("00" * dataLen + dialog.lineEditData.text().upper())[
            -dataLen * 2 :
        ]  # type: str
        if dialog.radioPrevContinuity.isChecked():
            ...
            if start == self.rowCount() - dialog.spinNbLines.value() - 1:
                ...
                addr_start = (
                    self.sxfile.sxItems[start - 1].addressValue()
        ...
        for i in range(start, start + dialog.spinNbLines.value(), 1):
            sx = SxItem(
                format, "00", SxItem.formatAddress(addr_start, format), "00", "00"
            )  # type: SxItem
            sx.updateData(data)
    ...
    def applyNewRowSize(
        self, newRowSize: int, selectedRowStart: int, selectedRowEnd: int
    ) -> None:
        """Adjust the size of the rows is the range (selectedRowStart,
        selectedRowEnd) with selectedRowEnd included"""
        self.sxfile.applyNewRowSize(
            newRowSize, selectedRowStart - 1, selectedRowEnd - 1
        )
        self.sxfile.syncEx()
        self.redisplayTable()
        self.sigDataModifiedChanged.emit(True)
