...
def str2hex(s: Optional[str]) -> List[int]:
    ...
    if len(norm_s) % 2 != 0:
        raise ValueError(
            'String contains an odd number of characters, can not convert to hex: "%s"'
            % norm_s
        )
    ret = []  # type: List[int]
...
class SxItemException(Exception):
    pass


class SxItemBadOutFormat(SxItemException):
    pass


class SxItemChecksumError(SxItemException):
    pass


class SxItemBadNewAddress(SxItemException):
    pass


class SxItemMissingData(SxItemException):
    pass


class SxItemBadFileFormat(SxItemException):
    pass


class SxItemBadOffset(SxItemException):
    pass
...
class SxItem:
    ...
    def __init__(
        self, format: str, data_qty: str, address: str, data: str, checksum: str
    ):
    ...
    def setContent(self, content: str, lineNb: int = -1) -> None:
        ...
        if len(content[4:]) != str2hexi(self.data_quantity) * 2:
            raise SxItemMissingData(
                "%sAddress %s, data is announced as %d bytes but is actually %d bytes!"
                % (
                    ("" if lineNb == -1 else "Line %d," % lineNb),
                    self.address,
                    str2hexi(self.data_quantity),
                    len(content[4:]) // 2,
                )
            )
    ...
    def merge(self, other: "SxItem") -> None:
        ...
        if not self.mergePossible(other):
            raise SxItemBadNewAddress(
                "Can not merge address %s, data len %d with new address %s"
                % (self.address, self.dataLen(), other.address)
            )
    ...
    def __repr__(self) -> str:
        return (
            self.format + self.data_quantity + self.address + self.data + self.checksum
        )
    ...
    def applyNewRowSize(self, newRowSize: int, itemStart: int, itemEnd: int) -> None:
        ...
        while idx <= itemEnd:
            if newRowSize > self.sxItems[idx].dataLen():
                ...
                if idx + 1 <= itemEnd and self.sxItems[idx].mergePossible(
                    self.sxItems[idx + 1]
                ):
